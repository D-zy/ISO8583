 ...
 * 工具类  安全加密
 */
class MmvkUtil(context: Context) {

    companion object {
        private var mInstance: MmvkUtil? = null
        private lateinit var mv: MMKV

        /**
         * 初始化MMKV,只需要初始化一次，建议在Application中初始化
         */
        fun getInstance(context: Context): MmvkUtil? {
            if (mInstance == null) {
                synchronized(MmvkUtil::class.java) {
                    if (mInstance == null) {
                        mInstance = MmvkUtil(context)
                    }
                }
            }
            return mInstance
